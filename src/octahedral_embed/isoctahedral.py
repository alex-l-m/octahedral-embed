...
def min_angle(mol, metal="Ir", conformer_index=0):
    """Calculate the minimum angle between the first 12 nearest neighbors of a metal atom."""
    coordlist = metal_neighbor_coords(mol, metal, conformer_index)
...
def isoctahedral(mol: Mol, conformer_index: int = 0) -> bool:
    return min_angle(mol, conformer_index=conformer_index) > 70
